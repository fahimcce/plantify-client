...
const PubliProfile = ({ userId }: { userId: string }) => {
  ...
  const isFollower = userData?.follower?.some(
    (item: string) => item === localUser?._id
  );
          ...
          <div className="flex flex-col gap-5 font-roboto_slab">
            <div className="flex-1">
              <div className="flex flex-col gap-2">
                ...
                <span className="text-sm md:text-lg font-medium">
                  Varified:{" "}
                  {userData?.verified === true ? "Verified" : "Not verified"}
                </span>
              </div>
            </div>
            <div>
              {!isLoading && localUser?.email !== userData?.email ? (
                <Button
                  className={`${isFollower ? "bg-pink-600" : "bg-secondary"}  text-white md:px-10`}
                ...
                <h3 className="md:text-lg font-semibold font-roboto_slab">
                  Your Total Post {post?.data?.length}
                </h3>
              )}
            </div>
          </div>
