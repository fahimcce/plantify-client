...
import { useGetPostByIdQuery } from "@/src/redux/features/post/postApi";
      ...
      {isLoading ? (
        ...
        <div className="max-w-[900px] px-3 mx-auto">
          ...
          <div className="relative h-80 w-full overflow-hidden rounded-lg">
            ...
            >
              <h1 className="text-white text-xl md:text-3xl font-bold px-3">
                {post?.title}
              </h1>
            </motion.div>
          </div>
              ...
              {post?.activity?.map(
                (item: any) =>
                  ...
                  item?.comment?.map((comment: string, idx: number) => (
                    <div
                      key={idx}
                      className="bg-gray-100 rounded-lg p-3 mt-4 flex gap-4"
                    >
                      <Image
                        ...
                        className="border p-[1px] size-24 rounded-full"
                      />
                      <div className="w-full">
                        <h2 className="font-semibold md:text-lg">
                          Name: <span>{item?.userId?.name}</span>
                        </h2>
